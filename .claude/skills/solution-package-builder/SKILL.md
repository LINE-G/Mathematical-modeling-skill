...
2. Create or refresh `paper/claim_evidence_map.json` so each final claim identifies its Qx, frozen value/source, supporting visual, robustness or validation evidence, applicable scope, limitation, and human decision ID.
3. Draft `results/Qx/reports/qx_solution_package_for_writer.md`.
4. Flag every proposed top-line claim with:
   - value and unit;
   - canonical source path and location;
   - robustness support;
   - decision ID for claim scope or rationale;
   - confidence/limitation.
5. Invoke one final choice card when package sign-off is missing:
   - keep;
   - downgrade;
   - drop.
6. Route the human answer to `modeler-decision-logger` as `package_signoff`.
7. Only after sign-off, generate `results/Qx/reports/frozen_numbers.json`.
8. Verify every numerical package claim resolves to the freeze and every judgment claim resolves to a human decision ID.
...
- Claim-evidence map distinguishes calibration, validation, robustness, and boundary evidence.
