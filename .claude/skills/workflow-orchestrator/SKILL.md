...
```json
{
  ...
  "artifacts": {
    ...
    "model_architecture": "planning/model_architecture.json",
    "evidence_plan": "planning/evidence_plan.json",
...
- when the solution has multiple modeling layers, architecture and evidence plans identify layer roles, handoffs, and a validation route.
...
- current `paper/claim_evidence_map.json` for paper-level claims.

## G5 — PAPER_SECTION_READY

Require the three writer rules, frozen-number sourcing, human-confirmed interpretation/claim scope, verified figures, and direct answers for every Qx. Final abstract writing is allowed only after all of its claims are frozen.

## G6 — FINAL_AUDIT_PASSED

Evaluate only in `submission`. Require passing consistency, completeness, QA, and `award-readiness-auditor` artifacts. The award-readiness audit is enabled by default in submission and may be skipped only by an explicit human decision recorded in the session configuration. Never infer that one auditor covers another.
...
- missing model architecture or evidence plan → `model-architecture-designer`;
...
- missing results discussion → `paper-results-discussion-writer`;
- abstract requested with frozen results → `paper-abstract-writer`;
...
- run consistency, completeness, QA, and the configured award-readiness audit once before final assembly.
...
- Multi-layer architectures have evidence plans before method screening and their paper claims have traceable validation.
