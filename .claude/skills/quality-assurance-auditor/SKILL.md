...
3. **Method quality**
   ...
   - A multi-stage architecture has explicit layer handoffs; complexity is justified by a necessary modeling responsibility.
   - Calibration is not presented as independent validation.
...
4. **Paper quality**
   ...
   - Every Qx has a direct answer, a usable output, and an applicable-range or limitation statement.
   - Abstract, results, conclusion, and `paper/claim_evidence_map.json` agree on core claims.
...
5. **Presentation**
   ...
   - The rendered document respects verified template requirements; historical styles are not mistaken for current rules.
...
- When configured, hand off to `award-readiness-auditor` after QA passes; it is an additional preparation check, not a replacement for QA.
