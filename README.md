...
> [!NOTE]
> **Update — this is now an assistant, not an autopilot.** The earlier version ran the whole contest end to end and left the user only clicking "confirm", which is closer to ghost-writing: it does not fit most contests' rules, and it does little for your own skills. This version hands the key judgments back to the user — the AI returns to a supporting role, and you stay in charge. The current release has 32 skills, including architecture, abstract, results-discussion, and award-readiness checks calibrated from 2004--2025 Huawei Cup papers. Those papers inform workflow quality; they are not copied and no prize is guaranteed. The previous full-auto version is kept intact on the [**`legacy-full-auto`**](https://github.com/zhnnky329/MathModeling-skills/tree/legacy-full-auto) branch.
...
workflow-orchestrator (reads interaction_mode + rigor_profile)
 ...
 ▼  model-architecture-designer (mathematical architecture + evidence route)
 ...
 ▼  paper-abstract-writer (after numbers are frozen)                    [ G5: PAPER_SECTION_READY ]
 ...
 ▼  award-readiness-auditor (additional preparation audit; no prize prediction)
 ▼  final assembly
```
...
Before any modeling begins, get the basics in order: what the problem is asking, what type each subquestion is, what data is available, a role-based mathematical architecture, and a single symbol table the whole team shares.
...
- **`model-architecture-designer`** — Defines the abstraction, layer handoffs, and evidence route before algorithm screening, so complexity has a mathematical responsibility.
...
The writer drafts the paper from the package and the frozen snapshot. Three independent auditors then check it: cross-file consistency, semantic completeness, and overall QA. A fourth award-readiness audit is enabled by default for submission preparation. If any required audit fails, the paper cannot be submitted.
...
- **`paper-results-discussion-writer`** — Writes result, comparison, interpretation, uncertainty, and applicable-boundary paragraphs from the claim-evidence map.
- **`paper-abstract-writer`** — Creates a result-free outline before freeze and a compact evidence-backed abstract after freeze.
...
- **`award-readiness-auditor`** — Adds a non-predictive high-standard check for question closure, mathematical substance, independent evidence, usable outputs, writing, and rendered layout.
...
Native plugin mode provides all 32 skills and their packaged workflow policy. To also place `CLAUDE.md`, `AGENTS.md`, Claude permissions/hooks, and standalone skill trees directly in a contest project, use project mode:
