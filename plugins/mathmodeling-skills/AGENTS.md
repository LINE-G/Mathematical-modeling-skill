...
- `award_readiness_audit`: defaults to `true` in `submission`; disabling it requires an explicit human choice and is not recommended for a final contest package.
...
- Before method screening, design a role-based model architecture and evidence plan when a subquestion has multiple layers or a nontrivial validation burden.
...
- Prefer a justified mechanism/estimation/decision architecture over an unmotivated stack of algorithm names. Every layer needs an input, output, mathematical responsibility, and check.
...
- Write chapters from a claim-evidence map. Results discussion must state result, comparison, interpretation, uncertainty, and applicable boundary. Write the final abstract only after the relevant numbers are frozen.
- Use the official current contest template for layout; historical excellent-paper styles are calibration references, not hard requirements.
- Mention eliminated methods only when the record helps explain a real trade-off; do not manufacture breadth.

# Award-Oriented Preparation

- `award-readiness-auditor` is an additional non-predictive preparation gate enabled by default in `submission`. It checks question closure, mathematical substance, evidence independence, decision usefulness, narrative quality, and rendered layout after the ordinary three audits pass.
- It may report `BLOCKER`, `MAJOR`, or `MINOR`, but it never promises a prize or replaces the human modeler's judgment.

# Verification

- In `lean`, verify the current gate and only the affected artifacts.
- In `submission`, verify all required artifacts, frozen-number lineage, figure rendering, references, the three independent audits, and the configured award-readiness audit.
