...
3. For each proposed visual record:
   ...
   - evidence role: architecture explanation, input characterization, comparison, result, robustness, or boundary.
...
- For a multi-stage solution, use one overview diagram only when it clarifies layer handoffs; do not duplicate it with decorative process charts.
- Reserve a result visual or exact table for each top-line claim that cannot be verified clearly in prose.
...
- Model architecture, result evidence, and robustness/boundary visuals have distinct purposes.
