...
6. For multi-stage architectures, define each stage's input/output schema, unit conversion, cached artifact, and failure propagation. A stage cannot be implicit in a final script.
7. Define the evidence matrix: baseline comparison plus the architecture-relevant ablation, independent simulation/theory check, and stress scenario where applicable. Mark tests that are not applicable with a reason instead of fabricating them.
8. Define the round output:
...
9. Write `code/Qx/qx_code_plan.md` for Python or `code/matlab/Qx/qx_code_plan.md` for MATLAB.
10. Hand off to the matching language generator.
...
- multi-stage artifact contracts and layer handoffs when applicable;
- comparable outputs and metrics;
- evidence matrix: baseline, ablation, independent check, and stress scenario where applicable;
...
- Multi-stage interfaces and planned validation artifacts are explicit.
