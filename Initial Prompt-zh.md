...
→ model-architecture-designer（多层模型先设计架构与证据路线）
...
- 多层模型必须明确每一层的输入、输出、数学职责和独立验证方式，不能靠算法名称堆叠制造“创新”。
- QA 没通过前，不要组装最终论文。
- 结果讨论必须同时说明结果、比较、解释、不确定性和适用边界；摘要应在数字冻结后定稿。
...
- `submission` 默认启用 `award_readiness_audit: true`；只有使用者明确记录取舍时才可关闭。
- `lean` 阶段不强制每轮长报告、成功日志、冻结数字或最终审计。
- `award-readiness-auditor` 是额外的高标准准备审查，不预测也不保证奖项。
