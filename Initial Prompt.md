...
→ model-architecture-designer (when the solution has multiple layers)
...
→ paper-results-discussion-writer
→ paper-abstract-writer (after results are frozen)
→ paper-polisher
→ reference-manager
→ consistency-auditor
→ completeness-auditor
→ quality-assurance-auditor
→ award-readiness-auditor
→ workflow-orchestrator
...
- For multi-stage models, make every layer's input, output, mathematical responsibility, and independent check explicit; algorithm-name stacking is not innovation.
- Do not assemble the final paper before QA passes.
- Results discussion must state result, comparison, interpretation, uncertainty, and applicable boundary; finalize the abstract only after freezing numbers.
...
- `submission` enables `award_readiness_audit: true` by default; disable it only with an explicit human decision.
