...
Read `../_references/huawei-cup-writing-and-layout-patterns.md` when polishing a contest manuscript; official current rules override historical patterns.
...
7. Apply high-density contest prose checks: each paragraph advances one mathematical or evidentiary point; remove ceremonial transitions and procedural diary language; state result, evidence, and boundary directly; keep the abstract concrete and shorter than the body-level explanation; ensure captions express the condition and takeaway.
...
- Treat a historical layout convention as a suggestion only; current template controls margins, fonts, cover, page count, and disclosure.
