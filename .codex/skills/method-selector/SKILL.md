...
- `planning/model_architecture.json` and `planning/evidence_plan.json` exist when the solution needs more than a single direct method.
...
- `planning/model_architecture.json` and `planning/evidence_plan.json` when present.
...
2. **Derive method requirements.**
   ...
   - Honor architecture roles and layer handoffs rather than treating each named algorithm as an independent candidate.
...
- Do not add algorithm layers merely to make the solution look advanced; prefer the simplest architecture that satisfies the required output and evidence plan.
